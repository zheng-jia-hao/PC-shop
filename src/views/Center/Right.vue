        ...
        <div class="order-content">
                ...
                <table class="order-item" v-for="order in records" :key="order.id">
                    <thead>
                        <tr>
                            <th colspan="5">
                                <span
                                    class="ordertitle">{{order.createTime}}&nbsp;&nbsp;&nbsp;&nbsp;订单编号：{{order.outTradeNo}}
                                    <span class="pull-right delete"><img src="./images/delete.png"></span></span>
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="(shop,index) in order.orderDetailList" :key="shop.id">
                            <td width="60%">
                            ...
                            <!-- rowspa属性指定td上下合并多少行 -->
                            <!-- template其实是浏览器自带的标签元素，它没有任何意义,也不会影响包裹内的元素 -->
            ...
            <div class="choose-order">
                ...
                <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" background
                    :current-page.sync="page" :page-size="orderDetails.size" layout="total, prev, pager, next"
    ...
    export default {
        ...
        methods: {
            async getOrderDetails() {
                let res = await this.$API.getOrderList(this.page, 3);
                if (res.code == 200) {
        ...
        computed: {
            records() {
                return this.orderDetails.records || []
            }
        }
    }
</script>
