          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(SaleAttr,index) in spuSaleAttrList" :key="index">
                ...
                <dd changepirce="0" @click="activeAttr(SaleAttrValue,SaleAttr.spuSaleAttrValueList)"
            ...
            <div class="cartWrap">
              <div class="controls">
                <!-- 这里选择使用change事件，因为计算属性性能太差 -->
                <input autocomplete="off" v-model="shopCount"
                  @change="parseInt(shopCount)?parseInt(shopCount):shopCount = 1" class="itxt">
                <a href="javascript:" @click="shopCountAdd(1)" class="plus">+</a>
                <a href="javascript:" @click="shopCountAdd(2)" class="mins">-</a>
              </div>
              <div class="add">
                <a href="javascript:" @click="AddShopCat">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
<script>
  ...
  export default {
    ...
    mounted() {
      this.$store.dispatch("getShopDetailAction", this.$route.params.id);
    ...
    methods: {
      ...
      async AddShopCat() {
        //从这里开始使用了 SHOPCART中的action，因为后面的结算界面也需要增加或减少商品数量

        this.$store.dispatch("AddShopinShopCartAction", {
          id: this.$route.params.id,
          count: this.shopCount
        })
        sessionStorage.setItem('SKUINFO_KEY', JSON.stringify(this.skuInfo))
        sessionStorage.setItem('ATTRLIST_KEY', JSON.stringify(this.spuSaleAttrList))
        this.$router.push('/addcartsuccess?count=' + this.shopCount)
